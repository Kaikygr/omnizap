          ...
          if (isGroupMessage) {
            ...
            try {
              ...
              if (cachedGroupData) {
                ...
                if (groupMetadata) {
                  if (this.redisClient) await this.redisClient.set(groupCacheKey, JSON.stringify(groupMetadata), 'EX', REDIS_TTL_METADATA_SHORT);
      ...
      if (jid) {
        try {
          ...
          if (metadata) {
            ...
            if (this.redisClient) await this.redisClient.set(cacheKey, JSON.stringify(metadata), 'EX', REDIS_TTL_METADATA_SHORT);
  ...
  async handleGroupsUpsert(groups) {
    ...
    for (const groupMetadata of groups) {
      ...
      if (jid) {
        try {
          const cacheKey = `${REDIS_PREFIX_GROUP}${jid}`;
          if (this.redisClient) await this.redisClient.set(cacheKey, JSON.stringify(groupMetadata), 'EX', REDIS_TTL_METADATA_SHORT);
    ...
    for (const chat of chats) {
      if (chat.id) {
        try {
          ...
          if (this.redisClient) await this.redisClient.set(cacheKey, JSON.stringify(chat), 'EX', REDIS_TTL_METADATA_SHORT);
    ...
    for (const contact of contacts) {
      if (contact.id) {
        try {
          ...
          if (this.redisClient) await this.redisClient.set(cacheKey, JSON.stringify(contact), 'EX', REDIS_TTL_METADATA_LONG);
    ...
    for (const msg of messages) {
      ...
      if (msg.key && msg.key.remoteJid && msg.key.id && this.redisClient) {
        ...
        try {
          ...
          if (this.redisClient) await this.redisClient.set(cacheKey, JSON.stringify(messageToStore), 'EX', REDIS_TTL_MESSAGE);
    ...
    for (const chatUpdate of updates) {
      if (chatUpdate.id) {
        try {
          ...
          if (this.redisClient) await this.redisClient.set(cacheKey, JSON.stringify(chatUpdate), 'EX', REDIS_TTL_METADATA_SHORT);
    ...
    for (const contactUpdate of updates) {
      if (contactUpdate.id) {
        try {
          ...
          if (this.redisClient) await this.redisClient.set(cacheKey, JSON.stringify(contactUpdate), 'EX', REDIS_TTL_METADATA_LONG);
